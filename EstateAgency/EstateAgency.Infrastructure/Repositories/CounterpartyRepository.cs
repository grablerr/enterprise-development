...
public class CounterpartyRepository(AppDbContext context) : IRepository<Counterparty>
{
    ...
    {
        var counterparty = await context.Counterparties.FindAsync(id) ?? throw new KeyNotFoundException($"Counterparty with Id {id} not found.");
    ...
    {
        var toUpdate = await context.Counterparties.FindAsync(counterparty.Id) ?? throw new KeyNotFoundException($"Estate with Id {counterparty.Id} not found.");
